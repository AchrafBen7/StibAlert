...
struct HaltesParLigneView: View {
    ...
    var onClose: () -> Void // ✅ pour revenir automatiquement
    ...
    var body: some View {
        ...
        .navigationTitle("Arrêts ligne \(lineId)")
        .onAppear {
    ...
    func toggleFavori(arretId: String, userId: String, token: String) {
        ...
        URLSession.shared.dataTask(with: request) { _, _, error in
            DispatchQueue.main.async {
                if let error = error {
                    print("❌ Erreur favoris :", error.localizedDescription)
                    showTemporaryToast(message: "Erreur lors de l'ajout")
                } else {
                    if favorisLocaux.contains(arretId) {
                        favorisLocaux.remove(arretId)
                        showTemporaryToast(message: "Retiré des favoris ❌")
                    } else {
                        favorisLocaux.insert(arretId)
                        showTemporaryToast(message: "Ajouté aux favoris ✅")
                        DispatchQueue.main.asyncAfter(deadline: .now() + 0.6) {
                            onUpdateFavoris()
                            onClose() // ⬅️ fermeture de la sheet automatiquement
                        }
                    }
