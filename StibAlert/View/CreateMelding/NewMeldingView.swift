...
struct NewMeldingView: View {
    ...
    private func computeNearbyHaltes() -> [HalteModel] {
        guard let userLocation = locationManager.userLocation else {
            print("[DEBUG] ❌ Gebruikerspositie niet beschikbaar")
            return []
        }
        
        print("[DEBUG] 📍 Positie gebruiker : \(userLocation.latitude), \(userLocation.longitude)")
        print("[DEBUG] 🧮 Totaal aantal beschikbare stops : \(arretsVM.arrets.count)")
        ...
        let result: [HalteModel] = arretsVM.arrets.compactMap { halte -> HalteModel? in
            ...
            if distance <= 1300 {
                if seen.contains(halte.nom) {
                    print("[DEBUG] 🔁 Genegeerd duplicaat : \(halte.nom)")
                    return nil
                }
                ...
                print("[DEBUG] ✅ Stop '\(halte.nom)' à \(Int(distance)) m")
                return updatedHalte
            } else {
        ...
        print("[DEBUG] 📌 Stops in de buurt gevonden : \(result.count)")
        return result
    }
    ...
    var body: some View {
        ...
        .alert(isPresented: $showSuccessAlert) {
            Alert(
                title: Text("Succès"),
                message: Text("De melding is aangemaakt."),
    ...
    private var formSection: some View {
        VStack(alignment: .leading, spacing: 20) {
            ...
            if arretsVM.arrets.isEmpty {
                HStack(spacing: 10) {
                    ProgressView()
                    Text("Zoek naar haltes in de buurt...")
                        .font(.footnote)
                        .foregroundColor(.gray)
                }
            } else {
                VStack(spacing: 6) {
                    ...
                    if computeNearbyHaltes().count > 5 {
                        Button(action: {
                            ...
                            Text("Bekijk alle haltes in de buurt")
                                .font(.footnote)
                                .foregroundColor(.blue)
                                .padding(.top, 4)
                        }
    ...
    private var envoyerButtonSection: some View {
        Button {
            guard let typeProbleme = selectedProbleem?.rawValue else {
                viewModel.errorMessage = "Selecteer een type probleem."
                return
            }
