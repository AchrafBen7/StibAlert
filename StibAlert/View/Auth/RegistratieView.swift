    ...
    var body: some View {
        VStack {
            ...
            VStack(spacing: 16) {
                ...
                TextField("Volledige naam", text: $nom)
                    .padding()
                ...
                NavigationLink(destination: ConnexionView(authVM: authVM)) {
                    Text("Heb je al een account? Aanmelden")
                        .font(.footnote)
                        .foregroundColor(.blue)
                        .underline()
                }
        ...
        .alert("✅ Code verzonden", isPresented: $showAlert) {
            Button("OK") {
                navigateToActivation = true
            }
        } message: {
            Text("Er is een code verzonden naar \(email).")
        }
    }
}
